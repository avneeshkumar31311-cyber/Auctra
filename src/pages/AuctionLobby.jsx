...
export default function AuctionLobby() {
  const { auction, startAuction } = useAuction();
  const navigate = useNavigate();

  // If there's no item, redirect to seller dashboard / home
  useEffect(() => {
    if (!auction.item && auction.status === 'idle') {
      navigate("/");
    }
  }, [auction.item, auction.status, navigate]);

  // If status becomes live, instantly redirect!
  useEffect(() => {
  ...
  if (!auction.item) return null;

  const count = auction.acceptedBidders.length;
  ...
  return (
    <div className="min-h-screen bg-black pt-32 px-6 flex flex-col items-center">
      ...
      {/* Lobby Header */}
      <div className="max-w-4xl w-full text-center mb-16 animate-in fade-in slide-in-from-bottom-5 duration-700">
        ...
        <p className="text-zinc-400 font-sans text-lg max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-2 gap-6 p-6 bg-black/50 rounded-2xl border border-zinc-800/50">
             <div>
               ...
               <p className="text-xl font-sans text-yellow-500 font-light">
                 {auction.item.suggestedBasePrice ? `₹${auction.item.suggestedBasePrice.amount.toLocaleString('en-IN')}` : `₹${parseFloat(auction.item.marketValue || 0).toLocaleString('en-IN')}`}
               </p>
             </div>
             <div>
          ...
          <div>
            ...
            <div className="bg-zinc-950 border border-zinc-800/80 rounded-3xl p-6 min-h-[300px] relative overflow-hidden">
               ...
               <div className="flex flex-wrap gap-4 relative z-10">
                 <AnimatePresence>
                   {auction.acceptedBidders.map((bidder, i) => (
                     <motion.div
                       key={bidder}
                     ...
                     >
                       <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${gradients[i % gradients.length]} flex items-center justify-center shadow-inner`}>
                         <span className="text-white font-serif text-lg leading-none">{bidder.charAt(0)}</span>
                       </div>
                       <div className="flex-1 overflow-hidden">
                         <p className="text-white text-sm font-bold truncate">{bidder}</p>
          ...
          <div className="mt-12 bg-zinc-900/50 p-8 rounded-3xl border border-zinc-800 backdrop-blur-sm">
            <div className="flex flex-col sm:flex-row items-center gap-6 justify-between">
              <div>
                ...
                {isReady ? (
                  <p className="text-white font-serif text-xl">Quorum reached. Ready to launch.</p>
                ) : (
